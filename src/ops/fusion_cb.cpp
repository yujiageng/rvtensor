...
CPUFusionCBOp::sptr CPUFusionCBOp::create(ConvParam conv_param,
                             BatchNormParam bn_param,
                             RamTensor::sptr input,
                             RamTensor::sptr output,
                             FlashTensor::sptr weight,
                             FlashTensor::sptr bias) {
  CPUFusionCBOp::sptr ptr = std::make_shared<CPUFusionCBOp>(conv_param,
                                                    bn_param,
                                                    input, output,
                                                    weight, bias);
  // ptr->checkOutputDims();
...
inline CPUFusionCBOp::CPUFusionCBOp() : Operation({}, {}),
                                conv_param_({0, 0, 1, 1, 0, 0, false}),
                                bn_param_({{}, {}, 0.001}),
                                weight_(nullptr), bias_(nullptr) {}

inline CPUFusionCBOp::CPUFusionCBOp(ConvParam conv_param,
                            BatchNormParam bn_param,
                            RamTensor::sptr input,
                            RamTensor::sptr output,
                            FlashTensor::sptr weight,
